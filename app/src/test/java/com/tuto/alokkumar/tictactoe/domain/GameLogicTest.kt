...
class GameLogicTest {
    ...
    fun `test win detection 2D`() {
        val logic = GameLogic(AiDifficulty.HARD, BoardSize(3, 3, 1, 3))
        logic.makeMove(0) // X
        logic.makeMove(3) // O
        logic.makeMove(1) // X
        logic.makeMove(4) // O
    ...
    fun `test HARD mode on 4x4`() {
        val logic = GameLogic(AiDifficulty.HARD, BoardSize(4, 4, 1, 4))
        val time = measureTimeMillis {
    ...
    fun `test HARD mode on 3x3x3`() {
        val logic = GameLogic(AiDifficulty.HARD, BoardSize(3, 3, 3, 3))
        val time = measureTimeMillis {
            val move = logic.getBestMove('X')
            assertNotNull(move)
        }
